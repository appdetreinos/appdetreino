-- Migration 0042_FIX_TRIGGER_CRASH_FINAL.sql
-- RESOLUÇÃO DEFINITIVA: Remove o trigger que está causando erro de null value no trainer_id

-- 1. Removemos o trigger e a função que tenta criar student_profiles automaticamente sem trainer_id
-- Isso impede que a criação de profiles dispare erros de constraint
...
-- 3. Vinculamos esses alunos ao treinador correto de forma explícita
...
FROM public.student_invites si
WHERE si.status = 'accepted' 
AND si.accepted_// a la l'ancien code, on a:
AND si.accepted_by IS NOT NULL
